...
function StoreCard({ item: st, onPress }) {
    ...
    <TouchableOpacity style={s.card} onPress={onPress} activeOpacity={0.85}>
      ...
      <View style={{ flex: 1, gap: 4 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md }}>
          {(st.rating > 0 || st.total_reviews > 0) && (
            <StarRow rating={st.rating} total={st.total_reviews} />
          )}
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
...
export default function StoresScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.bg }}>
      {/* Header */}
      <View style={s.header}>
        <Text style={s.title}>Cat Stores</Text>
        <Text style={s.sub}>Browse stores and order for your feline</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <StoreCard
              item={item}
              onPress={() => router.push(`/store-detail/${item.id}`)}
            />
          )}
          ListEmptyComponent={
            <View style={s.empty}>
              ...
              <Text style={s.emptyText}>
                No stores found{query ? ` for "${query}"` : ''}
              </Text>
              <Text style={[s.emptyText, { fontSize: FontSize.sm, marginTop: 4 }]}>
                Stores appear here after admin approval
              </Text>
            </View>
          }
        />
...
const s = StyleSheet.create({
  ...
  iconBox:    { width: 48, height: 48, borderRadius: Radius.lg, backgroundColor: Colors.amberBg, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  name:       { fontFamily: FontFamily.bodyBold, fontSize: FontSize.md, color: Colors.text },
  meta:       { fontFamily: FontFamily.bodyRegular, fontSize: FontSize.xs, color: Colors.textMuted, flexShrink: 1 },
